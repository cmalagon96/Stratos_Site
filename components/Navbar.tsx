...
export default function Navbar() {
  ...
  // P3-03: Initialize activeSection to the first nav link's id so the nav
  // has a sensible default on mount before IntersectionObserver fires.
  ...
  // P1-05: Close menu and return focus to hamburger button
  const closeMenu = useCallback(() => {
    setMenuOpen(false);
    // Return focus to the hamburger after AnimatePresence exit
    requestAnimationFrame(() => hamburgerRef.current?.focus());
  }, []);

  // P1-05: Escape key handler
  useEffect(() => {
    if (!menuOpen) return;
  ...
  // P1-05: Focus trap — keep Tab/Shift+Tab inside the mobile menu while open
  useEffect(() => {
  ...
  return (
    <header ref={navRef} className="fixed left-0 top-0 z-50 w-full">
      <nav className={`transition-all duration-500 ${navBg}`} role="navigation" aria-label="Main navigation">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3 md:px-12">

          {/* Logo */}
          <a href="#top" className="group shrink-0" aria-label="Stratos Strategies home">
      ...
      {/* P1-05: Mobile fullscreen menu — dialog semantics + focus trap */}
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ref={menuRef}
